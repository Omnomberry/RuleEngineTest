...
public class SQLWriter {
     ...
     * @param password
     * @param dbName
     *            the name of the database we are using, for example the ones
     *            we used early on: SpectreSoft or newspec. If none
     *            used/required, just pass an empty String.
     * @param writeAll
     *            true: writes all rows with REPLACE INTO. false: if primary
     *            key already existing, do nothing (INSERT IGNORE used).
     * @throws SQLException
     */
    ...
    /**
     * Insert new line into table problem, if you want to leave any field
     * empty, call with "NULL" for String or null for Boolean. Note that name
     * is primary id and cant be "NULL" or null.
     * 
     * @param name
    ...
    public void insertNewProblem(Integer problemID, String name, String dir,
        ...
        String[] values = { problemID.toString(), "'" + name + "'",
                dir == null ? "NULL" : getCharOfString(dir),
                getCharOfString(sparseString), "1", // TODO autoapproved ?
     ...
     * @param lowerProblem
     *            String name of problem that is the lower part of the
     *            reduction
     * @param upperProblem
     *            String name of problem that is the upper part of the
     *            reduction
    ...
    public void insertNewGraphclass(Integer gcID, String type, String name,
        ...
        String[] values = { gcID.toString(), "'" + type + "'",
                getCharOfString(correctBackslashes(name)), "1", // TODO
                                                                // autoapproved
                                                                // ?
                "NULL", "NULL", "'system'", getDateTime(), computedString };
    ...
    public void insertNewGraphclassNote(Integer noteID,
        ...
        String[] values = { noteID.toString(), parentGraphclass.toString(),
                ...
                "'" + correctQuotations(correctBackslashes(text)) + "'" };
    ...
    public void insertNewGraphclassSmallgraphDerivation(
        ...
        String[] values = { gcsgDerivationID.toString(),
                graphclassID.toString(),
                getCharOfString(correctBackslashes(smallgraphID)) };
    ...
    public void insertNewProperty(String values) throws SQLException {
        ...
        String sql = m_insertIgnoreOrReplace + "into " + m_dbName +
                "property" + " values("; // Header of SQL call
    ...
    public void insertNewSmallgraph(String name, Integer sgID, String link,
            String type, Integer nodes, String definition) throws SQLException {

        String[] values = { getCharOfString(correctBackslashes(name)),
                sgID.toString(),
                link == null ? "NULL" : getCharOfString(link),
                nodes == null ? "NULL" : nodes.toString(),
                getCharOfString(type), "NULL", "NULL",
                definition == null ? "NULL" : getCharOfString(definition),
                "1", // ?
    ...
    /**
     * Inserts new or updates already existing Smallgraph. This is needed
     * because some Graphs are written as Complement before they are defined
     * themselves. Img blob is auto Null for now, everything else is not Null.
     * This is referenced by forbidden, alias, hmt_family, smallmembers,
     * gc_sg_derivation and by itself(complement).
     * 
     * @param sgID
     * @param name
     ...
     * @throws SQLException
     */
    public void insertUpdateNewSmallgraph(String name, Integer sgID,
            String link, String type, Integer nodes, String definition)
            throws SQLException {
        ...
        String[] values = { getCharOfString(correctBackslashes(name)),
                ...
                getCharOfString(type), "NULL", "NULL",
                definition == null ? "NULL" : getCharOfString(definition),
                "1", "NULL", "NULL", "'system'", getDateTime() };
    ...
    public void insertNewSgSgDerivation(Integer sgsgID, String baseSgID,
        ...
        String[] values = { sgsgID.toString(),
                getCharOfString(correctBackslashes(baseSgID)),
    ...
    public void insertNewAlias(Integer aliasID, String sgName, String alias)
        ...
        String[] values = { aliasID.toString(),
                getCharOfString(correctBackslashes(sgName)),
    ...
    public void insertNewHmtFamily(Integer hmtFamilyID, String sgID,
        ...
        String[] values = {
                hmtFamilyID.toString(),
                getCharOfString(correctBackslashes(sgID)),
    ...
    public void insertNewHmtGrammar(Integer hmtGrammarID, String name,
        ...
        String[] values = { hmtGrammarID.toString(),
                getCharOfString(correctBackslashes(name)),
    ...
    /**
     * Convenience function for Smallgraphs to keep backslashes of
     * Smallgraphnames.
     * 
     * @param text
     *            String to be corrected
    ...
    /**
     * All another value String to the pool. Don't mix tables into the same
     * pool ! If you want to pool the inserts for a new table make sure the
     * pool is empty by calling {@link #executeSQLInsert() executeSQLInsert()}
     * 
     * @param table
     * @param values
     * @throws SQLException
    ...
    /**
     * Executes a single INSERT sql statement into the given table name with
     * the given values
     * 
     * @param table
    ...
    /**
     * Executes a single INSERT OR UPDATE sql statement into the given table
     * name with the given values
     * 
     * @param table
     *            which table to write to.
     * @param values
     ...
     * @throws SQLException
     */
    public void executeSingleSmallGraphSQLINSERTUPDATE(String table,
            String[] values) {
        String sql = "INSERT INTO " + m_dbName + table +
                " (sg_name, sg_id, link, nodes, type, image," +
                " expl, definition, approved, approved_at, " +
                "approved_by, entered_by, entered_at) " + "VALUES("; // Header
                                                                     // of
                                                                     // SQL
                                                                     // call
        ...
        try {
            ...
            sql = sql + ")" + m_insertOption; // End SQL call
            sql = sql +
                    " ON DUPLICATE KEY UPDATE sg_name=VALUES(sg_name), " +
    ...
    /**
     * Executes a single UPDATE sql statement for updating either image or
     * explanation of a smallgraph entry. This method should only be used by
     * UpdateSmallGraphs Class
     * 
     * @param type
     *            boolean value: 1 for image, 0 for explanation
     * @param sgname
     *            smallgraphname of smallgraph that should be updated
     * @param value
     *            updatevalue
     * @throws SQLException
     */
    public void executeSingleSmallGraphSQLUPDATE(boolean type, String sgname,
            String value) {
        String sql = null;
        if (type) {
            sql = "UPDATE smallgraph SET image = " + getCharOfString(value) +
                    " WHERE sg_name = " +
                    getCharOfString(correctBackslashes(sgname));
        } else {
            sql = "UPDATE smallgraph SET expl = " + getCharOfString(value) +
                    " WHERE sg_name = " +
                    getCharOfString(correctBackslashes(sgname));
        }
     ...
     * delete table and recreate it (very fast). If there are foreign
     * references to it, there will be an sqlexception. If you are sure there
     * wont be problems with the integrity, you can execute SET
     * FOREIGN_KEY_CHECKS=0 before and SET FOREIGN_KEY_CHECKS=1 after
